from norse.torch.benchmark.lif import lif_feed_forward_benchmark
...
if __name__ == "__main__":
    ...
    for features in feature_sizes:
        input_spikes = torch.from_numpy(
            (np.random.uniform(size=(n_time_steps, batch_size, features)) > 0.5)
        ).float()
        dts = benchmark(
