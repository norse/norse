...
def logical_and_test():
    z = logical_and(torch.from_list([1, 0, 0, 1]), torch.from_list([1, 0, 0, 0]))
    np.testing.assert_equal(z.numpy(), np.array([1, 0, 0, 0]))


def logical_or_test():
    z = logical_or(torch.from_list([1, 0, 0, 1]), torch.from_list([1, 0, 0, 0]))
    np.testing.assert_equal(z.numpy(), np.array([1, 0, 0, 1]))


def logical_xor_test():
    z = logical_xor(torch.from_list([1, 0, 0, 1]), torch.from_list([1, 0, 0, 0]))
    np.testing.assert_equal(z.numpy(), np.array([0, 0, 0, 1]))


def posedge_detector_test():
    z = torch.from_list([1, 0, 0, 1])
    z_prev = torch.from_list([0, 0, 1, 1])
