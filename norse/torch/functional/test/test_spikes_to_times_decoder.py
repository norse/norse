...
def test_zero_input():
    assert torch.allclose(
        ToSpikeTimes.apply(torch.zeros(10, 1, 1), 10), torch.as_tensor(torch.inf)
    )


def test_ones_input():
    assert torch.allclose(
        ToSpikeTimes.apply(torch.ones(10, 1, 1), 10),
...
def test_single_spike_input_with_time():
    ...
    test_input[2, :, :] = 1.0
    result = ToSpikeTimes.apply(test_input, 1)
...
def test_backward():
    ...
    result = ToSpikeTimes.apply(test_input, 1).sum()
    result.backward()
