...
def test_ExpFilter_forward_parameter_one_and_ones_weights_without_bias():
    ...
    filter_layer = ExpFilter(feature_size, output_size, tau_filter_inv=tau_filter_inv, input_weights=init_weights, bias=False)
...
def test_ExpFilter_forward_ones_weights():
    ...
    init_weights = torch.ones(feature_size, output_size)
    tau_filter_inv = 1 # => parameter = exp(-1e-3) 
    filter_layer = ExpFilter(feature_size, output_size, tau_filter_inv=tau_filter_inv, input_weights=init_weights, bias=True)
    for name, param in filter_layer.named_parameters():
        if name == 'linear.bias':
            bias = param.item()
    model_parameter = torch.exp(torch.tensor([-1e-3])).item()
...
def test_ExpFilterforward_():
    ...
    output_size = 1
    tau_filter_inv = 1 # => parameter = exp(-1e-3) 
    filter_layer = ExpFilter(feature_size, output_size, tau_filter_inv=tau_filter_inv, bias=True)
    for name, param in filter_layer.named_parameters():
    ...
    for i in range(prediction.shape[0] - 1):
        prediction[i + 1] += prediction[i] * model_parameter + data_after_linear_layer[i + 1]
    actual = filter_layer(data)
    assert torch.allclose(actual, prediction)
...
def test_ExpFilter_forward_few_inputs():
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            data_after_linear_layer[i, j] = torch.mm(data[i, j:j+1], weight.T) + bias
