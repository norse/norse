    ...
    def forward(self, input_tensor: torch.Tensor, state: Optional[Any] = None):
        ...
        if self.config:
            ...
            for state_variable in state: 
                sample_args += (torch.randn(state_variable.shape, device=state_variable.device, requires_grad=True),) # account for state

            if self.activation_sparse is not None and input_tensor.is_sparse:
                raise NotImplementedError("Cuda Graph version for sparse activation not yet implemented!")
            else:
